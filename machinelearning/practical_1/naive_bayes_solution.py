"""chine Learning Practical 1: Naive Bayes for text classification
...
def experiment(args):
  ...
  for label,file in test_files:
    ...
    # todo()
    neg_score = log(count_class["neg"]+args["alpha"]-1) \
                - log(num_training_files+(args["alpha"]-1)*2)
    pos_score = log(count_class["pos"]+args["alpha"]-1) \
                - log(num_training_files+(args["alpha"]-1)*2)
    ...
    for line in open(file):
      for token in line.split():
        if not args["stopwords"] or token not in stopwords:
          ...
          # todo()
          neg_score += (log(count_neg_words[token] + args["beta"]-1) \
                       - log(neg_words+(args["beta"]-1)*num_unique_words))
          pos_score += (log(count_pos_words[token] + args["beta"]-1) \
                       - log(pos_words+(args["beta"]-1)*num_unique_words))
